...
func (s *Selector) Accept(visitor Visitor) {
	if s.Elements != nil {
		newElements := make([]*Element, len(s.Elements))
		for i, el := range s.Elements {
			visited := visitor.Visit(el)
			if v, ok := visited.(*Element); ok {
				newElements[i] = v
			} else {
				// If visitor.Visit returns something else, or nil, decide handling.
				// For now, assume it returns *Element or the original element if not changed.
				newElements[i] = el
			}
		}
	...
	if s.ExtendList != nil {
		...
		for i, item := range s.ExtendList {
			newExtendList[i] = visitor.Visit(item) // Assume item is visitable
		}
		s.ExtendList = newExtendList
	}

	if s.Condition != nil {
		s.Condition = visitor.Visit(s.Condition)
	}
}
...
func (s *Selector) GenCSS(context any, output *CSSOutput) { // CSSOutput from go_parser
	...
	for _, element := range s.Elements {
		element.GenCSS(context, output) // Element must have GenCSS taking go_parser.CSSOutput
	}
}
