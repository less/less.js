...
less.Parser.fileLoader = function (originalHref, currentFileInfo, callback, env, newVars) {
    ...
    if (currentFileInfo && currentFileInfo.currentDirectory && !/^\//.test(originalHref)) {
        originalHref = less.path.join(currentFileInfo.currentDirectory, originalHref);
    }

    var href = originalHref;

    var path = less.path.dirname(href);
    ...
    try {
        ...
        callback({ type: 'File', message: "'" + less.path.basename(href) + "' wasn't found" });
        return;
    }
...
//    options.sourceMapBasepath = process.cwd();
    ...
    if (options.sourceMap === true) {
        ...
        options.sourceMap = less.path.basename(options.sourceMapFullFilename);
    }
