...
less.Parser = function Parser(env) {
    ...
    var imports = this.imports = {
        ...
        push: function (path, callback) {
            ...
            //
            less.Parser.importer(path, this.paths, function (root, sheet, env) {
...
tree.Import = function (path, imports) {
    ...
    if (! this.css) {
        imports.push(this.path, function (root, env) {
            if (! root) {
                throw new(Error)("Error parsing " + that.path);
            }

            // Only relevant for browser lib.
            if(env && env.lastModified)
            	that.lastModified = env.lastModified;
...
// We will keep a list of all imported stylesheets here.
less.watchedImports = {};

// Called by loadStyleSheet when a stylesheet has been parsed/reparsed.
function scanImports(root, path, base) {
	for(var i = 0; i < root.rules.length; i++) {
		var rule = root.rules[i];
		if(rule instanceof tree.Import) {
			// Keep a ref to the base Parser instance for the stylesheet that is included in
			// the page.
			rule.base = base;

			// Add this rule to watched imports.
			less.watchedImports[path + rule.path] = rule;

			// Check for any nested imports inside this import (lol).
			scanImports(rule.root, path + rule.path.replace(/[\w\.-]+$/, ''), base);
		}
	}
}
...
if (less.env === 'development') {
    ...
    less.watchTimer = setInterval(function () {
        ...
        // Time to iterate through all imports we are watching.
        for(var href in less.watchedImports)
    	{
    		// TODO. Is it ok to hardcode MIME? I think so because you can't define MIME in an
        	// @import.
    		loadStyleSheet({ href: href, href: href, type: "text/css" }, function(root, sheet, env) {
    			var watchedImport = less.watchedImports[href];
    			
    			// Has this imported stylesheet changed?
    			if(env.lastModified > watchedImport.lastModified) {
    				// Yup. Update the parser root for this imported stylesheet with new one.
    				watchedImport.root = root;
    				
    				// Update lastModified on the watched import to latest.
    				watchedImport.lastModified = env.lastModified;

    				// Get the base stylesheet and reload it.
    				// Note we don't pass in lastModified to createCSS().
    				// This is because the stylesheet on the page itself did not change.
    				var base = watchedImport.base;
    				var baseSheet = base.sheet;
    				var css = base.toCSS();
    				createCSS(css, baseSheet);
    				
    				// ... instead we just force an update of the cached data, without 
    				// modifying the timestamp.
    				cache.setItem(baseSheet.href.replace(/\?.*$/, ''), css);
				}
    		});
    	}
    }, less.poll);
           ...
           (new(Date)(lastModified).valueOf() ===
            ...
            try {
                new(less.Parser)({
                    ...
                    try {
                    	// Only scan this parser if it's an actual stylesheet (not a fake one
                    	// less.Parser.importer gives us)
                    	// Will need this later.
                    	if(typeof(sheet.getAttribute) == "function")
                    	{
                    		root.sheet = sheet;
                    		scanImports(root, sheet.href.replace(/\?.*$/, '').replace(/[\w\.-]+$/, ''), root);
                    	}
