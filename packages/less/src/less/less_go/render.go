	...
	return func(input string, args ...any) any {
		...
		if len(args) == 1 {
			...
			if cb, ok := args[0].(func(error, any)); ok {
				...
				options = CopyOptions(renderCtx.context.GetOptions(), make(map[string]any))
				...
				if opts, ok := args[0].(map[string]any); ok {
					...
					options = CopyOptions(renderCtx.context.GetOptions(), opts)
				} else {
								...
								if r := recover(); r != nil {
									if !recursiveCallbackCalled {
										...
										if e, ok := r.(error); ok {
											...
											recursiveCallback(NewLessError(ErrorDetails{
						...
						if r := recover(); r != nil {
							if !callbackCalled {
								...
								if e, ok := r.(error); ok {
									...
									callback(NewLessError(ErrorDetails{
										Message: "render error",
									}, make(map[string]string), ""), nil)
								}
							}
