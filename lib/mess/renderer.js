...
mess.Renderer = function Renderer(env) {
    ...
    return {
        ...
        resolveConditions: function(definitions) {
            ...
            for (var i = 0; i < definitions.length; i++) {
                var zoom = definitions[i].selector.filters.filter(function(f) {
                ...
                if (currentZoom.neverValid()) {
                    definitions[i] = undefined;
                    continue;
                }

                if (negation) negatedZoom.intersection(negation);

                var normal = definitions[i].selector.filters.filter(function(f) {
                ...
                definitions[i].selector.filters = normal;
                definitions[i].selector.zoom = currentZoom;
            }
            return definitions.filter(function(f) { return f; });
        },

        get_map_properties: function(rulesets) {
            var properties = [];
