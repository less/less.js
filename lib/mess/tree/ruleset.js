        ...
        for (var i = 0; i < this.selectors.length; i++) {
            ...
            if (parentSelectors.length) {
                for (var j = 0; j < parentSelectors.length; j++) {
                    ...
                    var instance = Object.create(tree.Selector.prototype);
                    instance.elements = parent.elements.concat(selector.elements);
                    instance.attachment = selector.attachment || parent.attachment;
                    instance.filters = {};
                    for (var id in parent.filters) { instance.filters[id] = parent.filters[id]; }
                    for (var id in selector.filters) { instance.filters[id] = selector.filters[id]; }
                    instance.zoom = parent.zoom & selector.zoom;
                    instance.conditions = parent.conditions + selector.conditions;
                    instance.index = selector.index;

                    if (tree.Filter.sound(instance.filters)) selectors.push(instance);
                }
            }
            else {
                if (tree.Filter.sound(selector.filters)) selectors.push(selector);
            }
        }
