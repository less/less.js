...
mess.Renderer = function Renderer(env) {
    ...
    return {
                        ...
                        new mess.Parser(_.extend(_.extend({
                            ...
                            if (err) {
                                ...
                                try {
                                    group()(err, [
                                        results[i][0],
                                        // TODO: need to rework the whole function to remove cruft
                                        undefined,
                                        tree]);
                                    return;
                                } catch (e) {
        ...
        split_symbolizers: function(definitions) {
            ...
            for (var i = 0; i < definitions.length; i++) {
                definitions[i].symbolizers().forEach(function(sym) {
                    ...
                    if(!by_symbolizer[index]) {
                        by_symbolizer[index] = [];
                    }
                    by_symbolizer[index].push(
        ...
        get_map_properties: function(rulesets, env) {
            var properties = [];
        ...
        template: function(err, res, callback) {
            ...
            output.unshift(
                ...
                + '<!DOCTYPE Map[]>\n'
                + '<Map '
                + this.get_map_properties(rulesets, env)
