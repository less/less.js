...
// MockNode implements the new node interfaces for testing
type MockNode struct {
	...
	Self      *MockNode // For circular reference testing
...
func (m *MockNode) GetTypeIndex() int {
	return m.TypeIndex
}

func (m *MockNode) Accept(visitor any) {
	if m.AcceptFn != nil {
		m.AcceptFn(visitor)
	}
}
...
func (m *MockNode) GetLength() int {
	return m.length
}

// GetElement returns element at index for array-like behavior
func (m *MockNode) GetElement(i int) any {
...
func TestVisitHandlesNodeWithoutTypeIndex(t *testing.T) {
	...
	node := &MockNode{Type: "TestNode", TypeIndex: 0}
...
func TestVisitCachesVisitFunctions(t *testing.T) {
	...
	node := &MockNode{Type: "TestNode", TypeIndex: 1}
...
func TestVisitArray(t *testing.T) {
	...
	t.Run("visits all nodes in non-replacing mode", func(t *testing.T) {
		...
		nodes := []any{
			&MockNode{Type: "TestNode", TypeIndex: 1},
			&MockNode{Type: "TestNode", TypeIndex: 1},
		}
	...
	t.Run("handles array results in replacing mode", func(t *testing.T) {
		...
		arrayResult := &MockArrayLikeNode{
			Items: []any{
				&MockNode{Type: "Item1", TypeIndex: 2},
				&MockNode{Type: "Item2", TypeIndex: 3},
			},
		}
...
func TestNodeReplacementBehavior(t *testing.T) {
	t.Run("should replace node when isReplacing is true", func(t *testing.T) {
		newNode := &MockNode{Type: "NewNode", TypeIndex: 2}
		...
		mockNode := &MockNode{Type: "TestNode", TypeIndex: 1}
...
func TestCircularReferenceSafety(t *testing.T) {
	t.Run("should handle circular references safely", func(t *testing.T) {
		...
		var circularNode *MockNode
		circularNode = &MockNode{
			Type:      "CircularNode",
			TypeIndex: 1,
			AcceptFn: func(v any) {
				acceptCallCount++
