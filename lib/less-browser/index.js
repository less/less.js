...
export default function less(window, options) {
    const document = window.document;
    const less = lessFactory();

    less.options = options;
    const environment = less.environment;
    const FileManager = fileManagerFactory(options, less.logger);
    ...
    const cache = less.cache = options.cache || cacheFactory(window, options, less.logger);
    ...
    function bind(func, thisArg, ...curryArgs) {
        return function (...args) {
            return func.apply(thisArg, curryArgs.concat(args));
        };
    }
