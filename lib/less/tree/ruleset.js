...
tree.Ruleset = function (selectors, rules, filename, source) {
...
tree.Ruleset.prototype = {
    eval: function (env) {
        var ruleset = new(tree.Ruleset)(this.selectors, this.rules.slice(0), this.filename, this.source),
            that = this,
            wrap_eval = function (node) {
            try {
                return node.eval(env);
            } catch (e) {
                e.filename = e.filename || that.filename;
                e.source = e.source || that.source;
                throw e;
            }
        };
            ...
            for (var i = 0; i < ruleset.rules.length; i++) {
                if (ruleset.rules[i] instanceof tree.Import) {
                    Array.prototype.splice
                         .apply(ruleset.rules, [i, 1].concat(wrap_eval(ruleset.rules[i])));
                }
            }
        }
        ...
        for (var i = 0, rule; i < ruleset.rules.length; i++) {
            ...
            if (! (rule instanceof tree.mixin.Definition)) {
                ruleset.rules[i] = rule.eval ? wrap_eval(rule) : rule;
            }
        }
