...
func TestSelector(t *testing.T) {
	...
	t.Run("accept", func(t *testing.T) {
		mockFileInfo := map[string]any{"filename": "test.less"}
		mockIndex := 1
		visitor := &selVisitor{returnVal: NewElement("", "v", false, mockIndex, mockFileInfo, nil)}
		...
		// ExtendList replaced
		if len(sel.ExtendList) != 1 || sel.ExtendList[0] != visitor.returnVal {
			t.Errorf("Expected ExtendList[0] %v, got %v", visitor.returnVal, sel.ExtendList)
		}
		// Condition replaced
		if sel.Condition != visitor.returnVal {
			t.Errorf("Expected Condition %v, got %v", visitor.returnVal, sel.Condition)
		}
	})
