...
// Environment represents the environment interface
type Environment interface {
	GetFileManager(path, currentDirectory string, context map[string]any, environment Environment) FileManager
}

// FileManager represents the file manager interface
type FileManager interface {
	LoadFileSync(path, currentDirectory string, context map[string]any, environment Environment) *LoadedFile
	LoadFile(path, currentDirectory string, context map[string]any, environment Environment, callback func(error, *LoadedFile)) any
...
func NewImportManager(environment Environment) func(less any, context map[string]any, rootFileInfo *FileInfo) *ImportManager {
