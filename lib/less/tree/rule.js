...
tree.Rule.prototype.eval = function (context) {
    if(this.name.charAt(1) === '@') {
        var variable = context.frames[0]._variables[this.name];
        var refVar = context.frames[0]._variables[this.name.substr(1)];
console.log(refVar);
        var resolvedName = '@' + refVar.value.value[0].value[0].value;
