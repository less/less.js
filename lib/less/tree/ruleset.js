...
tree.Ruleset.prototype = {
    eval: function (env) {
        ...
        var ruleset = this.clone();
        // push the current ruleset to the frames stack
        env.frames.unshift(ruleset);

        var process = function(ruleset) {
        ...
        if (this.root) {
            ...
            this.evalImports(env, ruleset).then(function() {
                return process(ruleset);
            }, function(e) {
                deferred.reject(e);
            }).then(function(x) {
                deferred.resolve(x);
    ...
    evalImports: function(env, ruleset) {
        var deferred = when.defer();
        var i = 0;

        var iterator = function(from) {
            for (i = from; i < ruleset.rules.length; i++) {
                if (ruleset.rules[i] instanceof tree.Import) {
                    ruleset.rules[i].eval(env).then(function(rules) {
                        [].splice.apply(ruleset.rules, [i, 1].concat(rules));
                        var skip = (typeof rules.length == 'number') ? rules.length : 1;
                        iterator(i+skip);
                    });
                    return;
                }
            }
            deferred.resolve(ruleset.rules);
        };
        iterator(0);
        return deferred.promise;
    },
