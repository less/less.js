...
module.exports = function() {
    ...
    var normalFolder = path.join(__dirname, 'less');
    var bomFolder = path.join(__dirname, 'less-bom');
    ...
    function testSourcemap(name, err, compiledLess, doReplacements, sourcemap, baseFolder) {
        ...
        fs.readFile(path.join(__dirname, '/', name) + '.json', 'utf8', function (e, expectedSourcemap) {
    ...
    function testImports(name, err, compiledLess, doReplacements, sourcemap, baseFolder, imports) {
        ...
        fs.readFile(path.join(__dirname, 'less', name) + '.json', 'utf8', function (e, expectedImports) {
            if (e) {
    ...
    function testJSImport() {
        ...
        toCSS({}, require('path').join(__dirname, 'less', 'root-registry', 'root.less'), function(error, output) {
            if (error) {
    ...
    function globalReplacements(input, directory, filename) {
        ...
        return input.replace(/\{path\}/g, p)
            ...
            .replace(/\{pathhref\}/g, '')
            .replace(/\{404status\}/g, '')
            .replace(/\{nodepath\}/g, path.join(__dirname, '..', 'node_modules', '/'))
            .replace(/\{pathrel\}/g, path.join(path.relative(__dirname, p), '/')) 
        ...
        fs.readdirSync(path.join(baseFolder, foldername)).forEach(function (file) {
            ...
            queue(function() {
                toCSS(options, path.join(baseFolder, foldername + file), function (err, result) {
                    ...
                    fs.readFile(path.join(__dirname, 'css', css_name) + '.css', 'utf8', function (e, css) {
    ...
    function toCSS(options, path, callback) {
        ...
        options.filename = require('path').resolve(__dirname, path);
        ...
        if (options.plugin) {
            var Plugin = require(require('path').resolve(__dirname, options.plugin));
