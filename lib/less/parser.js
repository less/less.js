    ...
    return parser = {
        ...
        parsers: {
            ...
            mixin: {
                ...
                //
                call: function () {
                    var elements = [], e, c, args = [], kwargs = [], inkwargs, index = i, s = input.charAt(i), important = false;
                    ...
                    if ($('(')) {
                        inkwargs = false;
                        while(arg = $(this.variable) || $(this.expression)) {
                            if(typeof arg == 'string') {
                                value = expect(this.expression, 'expected expression');
                                inkwargs = true;
                                kwargs.push({ name: arg, value: value });
                                if(! $(',')) { break; }
                            }
                            else {
                                if (inkwargs) {
                                    error('named arguments must come after positional arguments');
                                }
                                args.push(arg);
                                if(! $(',')) { break; }
                            }
                    ...
                    if (elements.length > 0 && ($(';') || peek('}'))) {
                        return new(tree.mixin.Call)(elements, args || [], kwargs, index, env.filename, important);
                    }
                },
                ...
                //
                //     .rounded (@radius: 2px, @color, @shadow: 0px) {
                //        ...
                //     }
                //
