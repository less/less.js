...
class ExtendFinderVisitor extends Visitor {
  constructor() {
    ...
    this.extendKeyMap = [Object.create(null)];
  ...
  _visitAtRule(node) {
    ...
    this.extendKeyMap.unshift(Object.create(null));
    node.extendMap = this.extendMap.slice(0);
    node.extendKeyMap = this.extendKeyMap.slice(0);
  }

  _visitAtRuleOut() {
    this.extendMap.shift();
    this.extendKeyMap.shift();
  }
  ...
  visitRuleset(rulesetNode) {
    ...
    const extMap = this.extendKeyMap[0];
    ...
    const pushExtend = (node, path) => {
      this.hasExtend = true;
      css = this._joinPath([node.selector]);
      const target = [css, node.option, node.selector.elements];
    ...
    const addPath = (path, targets) => {
      ...
      targets.forEach(target => {
        ...
        if (!ext[key]) {
          ext[key] = [[css, path, target[1], target[2]]];
          // extMap[key] = target[2];
        } else {
          ext[key].push([css, path, target[1], target[2]]);
        }
      });
    };
...
class ProcessExtendsVisitor extends Visitor {
   ...
   */
  _createDerived(targetPath, findElements, extend) {
    ...
    targetPath.forEach(sel => {
      for (let o = 0; o < sel.elements.length; o++) {
        ...
        const currentValue = getValue(currentEl.value);
  ...
  _visitAtRule(node) {
    ...
    this.extendKeyMap = node.extendKeyMap;
  }
  ...
  visitRuleset(rulesetNode) {
    ...
    const addInsertion = (selString, basePath, findElements) => {
          ...
          if (selString.indexOf(key) > -1) {
            ...
            map[key].forEach((match, i) => {
              ...
              if (match[2] === 'all' || exactMatch) {
                ...
                const insertion = this._createDerived(basePath, newFind, match[1]);
                ...
                if (nextMatch.indexOf(key) === -1) {
                  insertion.forEach(path => {
                    addInsertion(nextMatch, path, newFind);
                  });
                }
              }
            });
    ...
    rulesetNode.paths.forEach(basePath => {
      ...
      addInsertion(selector, basePath, null);
    });
