        ...
        parsers: {
            ...
            primary: function () {
                ...
                while ((node = $(this.mixin.definition) || $(this.rule)    ||  $(this.ruleset) ||
                               ...
                               || $(/^[\s\n]+/)) {
                    node && root.push(node);
                    // If we are in development mode and on a browser
                    // (where the debugger can be installed), add location
                    // information to the elements that are being parsed

                    // XXX: we might want to have a debug_info setting
                    // somewhere as SASS does.
                    if(less.env === "development" && less.mode === "browser") {
                        // If information extraction is possible,
                        // we add a 'dbg_filename'
                        // (the full path of the file)
                        // and 'dbg_lineno'
                        // (the line where the definition occurs)
                        // attribute to the various nodes.

                        // XXX: We need these attributes to be present
                        // only on CSS Rulesets
                        // (therefore those coming out after eval())
                        // so we might try to do some filtering here.
                        if(node && node.index) {
                            node.dbg_filename = env.filename;
                            if(env.paths && env.paths.length > 0)
                                // XXX: this method to determine the full path
                                // might be wrong
                                node.dbg_filename =
                                    env.paths[env.paths.length-1] +
                                    env.filename;
                            var node_location = getLocation(node.index,
                                                            input);
                            if(node_location)
                                node.dbg_lineno = node_location.line;
                        }
                    }
