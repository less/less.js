...
func (ptc *ParseTreeClass) NewParseTree(root any, imports *less.ImportManager) *ParseTree {
	return &ParseTree{
		Root:    root,
		Imports: imports,
...
func (pt *ParseTree) ToCSS(options *ToCSSOptions) (*ToCSSResult, error) {
	...
	// Transform the tree using go_parser.TransformTree
	...
	if options != nil && options.Compress {
		compress = true
		less.DefaultLogger.Warn("The compress option has been deprecated. " +
	...
	if options != nil && options.SourceMap != nil {
		...
		if pt.sourceMapBuilder != nil {
			...
			if builder, ok := sourceMapBuilder.(interface {
				ToCSS(any, map[string]any, *less.ImportManager) (string, error)
			}); ok {
				generatedCSS, err := builder.ToCSS(evaldRoot, toCSSOptions, pt.Imports)
				if err != nil {
					return nil, less.NewLessError(less.ErrorDetails{
						Message: err.Error(),
					}, pt.Imports.Contents(), pt.Imports.RootFilename())
				}
				css = generatedCSS
