...
function formatLeft(val) {
    return ((val.slice(0,1) === "@")? "" : "@") + val;
}

function formatRight(val) {
    return ((val.slice(-1) === ";")? val : val +";");
}

less.modifyVars = function(record, reload) {
    var name, str;
    if (reload) {
        globalVars = {};
         for (name in record) {
            globalVars[formatLeft(name)] = formatRight(record[name]);
        }
        this.refresh(true);
    } else {
        str = session_cache;
        for (name in record) {
            str += formatLeft(name) + ":" + formatRight(record[name]);
        }
...
function loadStyleSheet(sheet, callback, reload, remaining) {
    ...
    xhr(href, sheet.type, function (data, lastModified) {
        // Loop through any global vars and
        // add them to the end of the loaded data
