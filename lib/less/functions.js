...
tree.functions = {
    ...
    extract: function(property, shorthand) {
        if (shorthand instanceof tree.Dimension) {
            return shorthand;
        } else if (shorthand && typeof(shorthand.length) === 'number') {
            var shorthandIndex;
            if (property instanceof tree.Dimension) {
                shorthandIndex = property.value;
                shorthandIndex--; // (1-based index)
            } else if (property instanceof tree.Anonymous) {
                var index;

                // See if the `property` is a recognized shorthand key:
                index = ['top', 'right', 'bottom', 'left'].indexOf(property.value);
                if (index === -1) index = ['top-left', 'top-right', 'bottom-right', 'bottom-left'].indexOf(property.value);
                if (index !== -1) {
                    if (shorthand.length === 3 && index === 3) {
                        // This is the only case where the math would be overly complicated:
                        // "extract(left, 1px 2px 3px)" should return "2px"
                        shorthandIndex = 1;
                    } else {
                        shorthandIndex = (index % shorthand.length);
                    }
                } else {
                    throw { type: "Argument", message: "unrecognized argument - 'property': " + property.value };
                }
            } else {
                throw { type: "Argument", message: "invalid argument - 'property': " + property };
            }

            return shorthand[shorthandIndex];
        } else {
            throw { type: "Argument", message: "invalid argument - 'shorthand':" + shorthand };
        }
    },
