...
carto.Renderer = function Renderer(env) {
    env = _(env).clone();
    env.validation_data = env.validation_data || false;
    ...
    return {
                ...
                _(m.Layer).chain()
                    ...
                    .each(function(l) {
                        ...
                        function(err, files) {
                            ...
                            var bn = path.basename(l.Datasource.file,path.extname(l.Datasource.file));
                            var prj = _(files).detect(function(f) {
                                return path.extname(f).toLowerCase() === '.prj' && f.indexOf(bn) >= 0;
                            });
                            if (prj) {
        ...
        localizeExternals: function(m, callback) {
            ...
            Step(function downloadExternals() {
                var group = this.group();
                var nonfile = ['postgis', 'occi', 'geos'];
                var optfile = ['ogr'];
                m.Layer.forEach(function(l) {
                    var next = group();
                    if (_(nonfile).include(l.Datasource.type)) return next();
                    if (!l.Datasource.file) {
                        if (_(optfile).include(l.Datasource.type)) {
                            return next();
                        } else {
                            return next(new Error('Datasource.file required.'));
                        }
                    }
        ...
        style: function(m, callback) {
            var that = this;
            Step(function loadStyles() {
                var group = this.group();
                var compile = function(s, cb) {
                    // Maintain a parsing environment from
                    // stylesheet to stylesheet, so that frames
                    // and effects are maintained.
                    var parse_env = _(that.env).chain()
                        .extend(this.env)
                        .extend({filename:s.id})
                        .value();
                    var time = +new Date();

                    if (env.benchmark) cb = _(cb).wrap(function(p, err, tree) {
                        console.warn('Parsing time: '
                            + ((new Date() - time))
                            + 'ms');
                        p.call(this, err, tree);
                    });

                    (new carto.Parser(parse_env)).parse(s.data, cb);
                };
