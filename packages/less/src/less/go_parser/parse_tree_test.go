...
func (m *MockSourceMapBuilder) ToCSS(root any, options map[string]any, imports *less.ImportManager) (string, error) {
	if m.css != "" {
		return m.css, nil
	}
...
func TestParseTreeConstructor(t *testing.T) {
	...
	mockImports := &less.ImportManager{}
...
func TestCompressWarningCheck(t *testing.T) {
	...
	// The actual warning would be logged via less.DefaultLogger.Warn in real usage
	mockRoot := &MockEvaluatedRoot{css: "body { color: red; }"}
	mockImports := &less.ImportManager{}
