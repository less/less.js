	...
	defer func() {
		if r := recover(); r != nil {
			if err, ok := r.(error); ok {
				...
				iv.error = less.NewLessError(less.ErrorDetails{
...
func (iv *ImportVisitor) VisitImport(importNode any, visitArgs *VisitArgs) {
	...
	if !css || inlineCSS {
		...
		frames := less.CopyArray(iv.context.Frames)
		context := less.NewEval(nil, frames)
...
func (iv *ImportVisitor) processImportNode(importNode any, context *less.Eval, importParent any) {
...
func (iv *ImportVisitor) onImported(importNode any, context *less.Eval, args ...any) {
	...
	if e != nil {
		if lessErr, ok := e.(*less.LessError); ok {
			if lessErr.Filename == "" {
...
func (iv *ImportVisitor) evalForImport(node any, context *less.Eval) any {
	// Call evalForImport method if it exists
	if n, ok := node.(map[string]any); ok {
		if method, hasMethod := n["evalForImport"]; hasMethod {
			if fn, ok := method.(func(*less.Eval) any); ok {
				return fn(context)
			}
		}
	}
