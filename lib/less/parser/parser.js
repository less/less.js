    ...
    return {
        ...
        parsers: parsers = {
            ...
            variableCall: function (parsedName) {
                var lookups, elements, important, i = parserInput.i,
                    inValue = !!parsedName, name = parsedName, args;
                ...
                if (name || (parserInput.currentChar() === '@' &&
                    ...
                    parserInput.$char('>');
                    elements = this.mixin.elements();

                    lookups = this.mixin.ruleLookups();
                    
                    if (!lookups) {
                        args = this.mixin.args(true);
                    }
                    console.log(name);
                    
                    if (!lookups && !elements && !parserInput.$str('()')) {
                        parserInput.restore('Missing \'[...]\' lookup in variable call');
                        return;
                    }

                    if (!inValue && lookups && parsers.important()) {
                        important = true;
                    }

                    var call;
                    if (elements) {
                        var elem = new tree.Element('', new tree.Variable(name, i, fileInfo), true, i, fileInfo);
                        elements.unshift(elem);
                        call = new(tree.mixin.Call)(elements, args, i, fileInfo, !lookups && important);
                    }
                    else {
                        call = new tree.VariableCall(name, i, fileInfo);
                    }
            ...
            mixin: {
                ...
                call: function (inValue) {
                    var s = parserInput.currentChar(), important = false,
                        index = parserInput.i, elements, args;
                    ...
                    if (elements) {
                        ...
                        if (inValue || parsers.end()) {
                            ...
                            if (lookups) {
                                ...
                                return new tree.Selector(elements, null, null, index, fileInfo);
                            }
                            else {
                                return mixin;
                            }
