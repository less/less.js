...
mess.Renderer = function Renderer(env) {
    ...
    return {
        ...
        template: function(err, res, callback) {
            ...
            var rulesets = _.flatten(_.map(stylesheets, function(rulesets) {
            ...
            _.each(m.Layer, function(l) {
                var id = '#' + l.id;
                var classes = (l['class'] || '').split(/\s+/g);

                var matching = _.filter(rulesets, function(ruleset) {
                    return ruleset.matches(id, classes);
                });
