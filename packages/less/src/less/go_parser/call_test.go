...
func (m *mockFunctionCaller) IsValid() bool {
	return m.valid
}

func (m *mockFunctionCaller) Call(args []any) (any, error) {
	if m.callFn != nil {
...
// Mock FunctionCallerFactory implementation
type mockFunctionCallerFactory struct {
	caller FunctionCaller
	err    error
}

func (m *mockFunctionCallerFactory) NewFunctionCaller(name string, context EvalContext, index int, fileInfo map[string]any) (FunctionCaller, error) {
	if m.err != nil {
		return nil, m.err
	}
	return m.caller, nil
...
func TestCallEval(t *testing.T) {
	...
	t.Run("should temporarily toggle mathOn for non-calc functions", func(t *testing.T) {
		...
		mockFactory := &mockFunctionCallerFactory{
			caller: mockCaller,
		}
	...
	t.Run("should evaluate args when creating new Call node", func(t *testing.T) {
		...
		call.CallerFactory = &mockFunctionCallerFactory{
			caller: &mockFunctionCaller{
				valid: false,
			},
		}
