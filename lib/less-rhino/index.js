...
function writeError(ctx, options = {}) {
    ...
    const message = formatError(ctx, options);
    throw new Error(message);
}

function loadStyleSheet(sheet, callback, reload, remaining) {
    const endOfPath = Math.max(name.lastIndexOf('/'), name.lastIndexOf('\\'));
    const sheetName = name.slice(0, endOfPath + 1) + sheet.href;
    const contents = sheet.contents || {};
    let input = readFile(sheetName);
    ...
    parser.parse(input, (e, root) => {
        if (e) {
            return writeError(e);
        }
        try {
            callback(e, root, input, sheet, { local: false, lastModified: 0, remaining }, sheetName);
        } catch (e) {
            writeError(e);
        }
    });
...
less.Parser.fileLoader = (file, currentFileInfo, callback, env) => {
    ...
    const path = less.modules.path.dirname(href);
...
function writeFile(filename, content) {
    const fstream = new java.io.FileWriter(filename);
    const out = new java.io.BufferedWriter(fstream);
    out.write(content);
    out.close();
}
    ...
    args = args.filter(arg => {
        ...
        switch (arg) {
            ...
            case 'source-map-output-map-file':
                if (checkArgFunc(arg, match[2])) {
                    options.writeSourceMap = sourceMapContent => {
