...
func (efv *ExtendFinderVisitor) Run(root any) any {
	...
	// Convert []any to []*go_parser.Extend for type consistency
	extends := make([]*go_parser.Extend, len(efv.allExtendsStack[0]))
	for i, ext := range efv.allExtendsStack[0] {
		if extend, ok := ext.(*go_parser.Extend); ok {
			extends[i] = extend
		}
	}
	if rootWithExtends, ok := root.(interface{ SetAllExtends([]*go_parser.Extend) }); ok {
...
func (efv *ExtendFinderVisitor) VisitRuleset(rulesetNode any, visitArgs *VisitArgs) {
	ruleset, ok := rulesetNode.(*go_parser.Ruleset)
	if !ok {
		return
	}
	...
	var i, j int
	var extend *go_parser.Extend
	allSelectorsExtendList := make([]*go_parser.Extend, 0)
	var extendList []*go_parser.Extend
	...
	for i = 0; i < ruleCnt; i++ {
		if extendRule, ok := rules[i].(*go_parser.Extend); ok {
	...
	for i = 0; i < len(paths); i++ {
		...
		var selExtendList []*go_parser.Extend
		
		if selectorWithExtends, ok := selector.(interface{ GetExtendList() []*go_parser.Extend }); ok {
			selExtendList = selectorWithExtends.GetExtendList()
		}

		if selExtendList != nil {
			extendList = make([]*go_parser.Extend, len(selExtendList))
		...
		if extendList != nil {
			clonedExtendList := make([]*go_parser.Extend, len(extendList))
...
type ProcessExtendsVisitor struct {
	...
	allExtendsStack   [][]*go_parser.Extend
...
func (pev *ProcessExtendsVisitor) Run(root any) any {
	...
	var rootAllExtends []*go_parser.Extend
	if rootWithExtends, ok := root.(interface{ GetAllExtends() []*go_parser.Extend }); ok {
	...
	pev.allExtendsStack = [][]*go_parser.Extend{newAllExtends}
	newRoot := pev.visitor.Visit(root)
	pev.checkExtendsForNonMatched(newAllExtends)
	return newRoot
}

func (pev *ProcessExtendsVisitor) checkExtendsForNonMatched(extendList []*go_parser.Extend) {
	...
	for _, extend := range extendList {
		if !extend.HasFoundMatches && len(extend.ParentIds) == 1 {
			...
			if !indices[key] {
				indices[key] = true
				less.Warn(fmt.Sprintf("WARNING: extend '%s' has no matches", selector))
			}
		}
	}
}

func (pev *ProcessExtendsVisitor) doExtendChaining(extendsList []*go_parser.Extend, extendsListTarget []*go_parser.Extend, iterationCount int) []*go_parser.Extend {
	var extendIndex, targetExtendIndex int
	var matches []any
	extendsToAdd := make([]*go_parser.Extend, 0)
	var newSelector []any
	var selectorPath []any
	var extend, targetExtend, newExtend *go_parser.Extend
			...
			if len(targetExtend.SelfSelectors) > 0 {
				...
				if len(matches) > 0 {
					...
					for _, selfSelector := range extend.SelfSelectors {
						...
						newExtend = go_parser.NewExtend(targetExtend.Selector, targetExtend.Option, 0, targetExtend.FileInfo(), infoMap)
						newExtend.SelfSelectors = newSelector

						// add the extend onto the list of extends for that selector
						if len(newSelector) > 0 {
							if selectorWithExtends, ok := newSelector[len(newSelector)-1].(interface{ SetExtendList([]*go_parser.Extend) }); ok {
								selectorWithExtends.SetExtendList([]*go_parser.Extend{newExtend})
							}
						}
	...
	for extendIndex = 0; extendIndex < len(allExtends); extendIndex++ {
		for pathIndex = 0; pathIndex < len(ruleset.Paths); pathIndex++ {
			...
			if len(selectorPath) > 0 {
				if selectorWithExtends, ok := selectorPath[len(selectorPath)-1].(interface{ GetExtendList() []*go_parser.Extend }); ok {
...
func (pev *ProcessExtendsVisitor) findMatch(extend *go_parser.Extend, haystackSelectorPath []any) []any {
	...
	var i int
	needleElements := extend.Selector.(*go_parser.Selector).Elements
	...
	for haystackSelectorIndex = 0; haystackSelectorIndex < len(haystackSelectorPath); haystackSelectorIndex++ {
		...
		var hackstackElements []*go_parser.Element
		if selector, ok := hackstackSelector.(*go_parser.Selector); ok {
		...
		for hackstackElementIndex = 0; hackstackElementIndex < len(hackstackElements); hackstackElementIndex++ {
			...
			if extend.AllowBefore || (haystackSelectorIndex == 0 && hackstackElementIndex == 0) {
				...
				if element, ok := haystackElement.(*go_parser.Element); ok {
			...
			for i = 0; i < len(potentialMatches); i++ {
				...
				targetCombinator = ""
				if element, ok := haystackElement.(*go_parser.Element); ok && element.Combinator != nil {
				...
				if !pev.isElementValuesEqual(needleElements[matched].Value, haystackElement.(*go_parser.Element).Value) {
					potentialMatch = nil
				} else if matched > 0 {
...
func (pev *ProcessExtendsVisitor) isElementValuesEqual(elementValue1, elementValue2 any) bool {
	...
	if attr1, ok1 := elementValue1.(*go_parser.Attribute); ok1 {
		if attr2, ok2 := elementValue2.(*go_parser.Attribute); ok2 {
	...
	if selector1, ok1 := val1.(*go_parser.Selector); ok1 {
		if selector2, ok2 := val2.(*go_parser.Selector); ok2 {
...
func (pev *ProcessExtendsVisitor) extendSelector(matches []any, selectorPath []any, replacementSelector any, isVisible bool) []any {
	...
	path := make([]any, 0)
	var matchIndex int
	var selector *go_parser.Selector
	var firstElement *go_parser.Element
	var match map[string]any
	var newElements []*go_parser.Element

	for matchIndex = 0; matchIndex < len(matches); matchIndex++ {
		match = matches[matchIndex].(map[string]any)
		selector = selectorPath[match["pathIndex"].(int)].(*go_parser.Selector)
		
		// Get replacement selector elements
		replacementSel := replacementSelector.(*go_parser.Selector)
		
		firstElement = go_parser.NewElement(
		...
		if match["pathIndex"].(int) > currentSelectorPathIndex && currentSelectorPathElementIndex > 0 {
			...
			if len(path) > 0 {
				if pathSel, ok := path[len(path)-1].(*go_parser.Selector); ok {
					currentSelector := selectorPath[currentSelectorPathIndex].(*go_parser.Selector)
		...
		newElements = make([]*go_parser.Element, 0)
		...
		if currentSelectorPathIndex == match["pathIndex"].(int) && matchIndex > 0 {
			...
			newSelector, err := go_parser.NewSelector(newElements, nil, nil, 0, nil, nil)
			if err == nil {
	...
	for i, currentValue := range path {
		if selector, ok := currentValue.(*go_parser.Selector); ok {
...
func (pev *ProcessExtendsVisitor) VisitMedia(mediaNode any, visitArgs *VisitArgs) {
	var mediaAllExtends []*go_parser.Extend
	if media, ok := mediaNode.(interface{ GetAllExtends() []*go_parser.Extend }); ok {
...
func (pev *ProcessExtendsVisitor) VisitAtRule(atRuleNode any, visitArgs *VisitArgs) {
	var atRuleAllExtends []*go_parser.Extend
	if atRule, ok := atRuleNode.(interface{ GetAllExtends() []*go_parser.Extend }); ok {
