...
tree.mixin.Call.prototype = {
    ...
    eval: function (env) {
        var mixins, mixin, args, rules = [], match = false, i, m, f, isRecursive, isOneFound, rule, matchFlag, envCLone, successCount = 0;

        args = this.arguments && this.arguments.map(function (a) {
            return { name: a.name, value: a.value.eval(env) };
        });

        matchFlag = new(tree.Rule)("@NoOtherMatches", tree.True);
        envCLone = new(tree.evalEnv)(env, [new(tree.Ruleset)([], [matchFlag], false)].concat(env.frames));
            ...
            if ((mixins = env.frames[i].find(this.selector)).length > 0) {
                ...
                for (m = 0; m < mixins.length; m++) {
                    ...
                    if (mixin.matchArgs(args, env)) {
                        if (!mixin.matchCondition || mixin.matchCondition(args, envCLone)) {
                            try {
                                ...
                                //}
                                matchFlag.value.value = [tree.False];
                                successCount++;
                            } catch (e) {
                ...
                if (match) {
                    ...
                    tree.functions["last-mixin-applications-number"].current = successCount;
                    return rules;
                }
            }
        }
