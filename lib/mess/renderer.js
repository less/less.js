    ...
    return {
        ...
        removeDuplicateSelectors: function(selectors) {
            ...
            outer: for (var i = 0; i < selectors.length; i++) {
                for (var j = result.length - 1; j >= 0; j--) {
                    if (result[j].includesFiltersOf(selectors[i])) {
                        ...
                        if (selectors[i].includesFiltersOf(result[j])) {
                            ...
                            result[j].zoom.union(selectors[i].zoom);
                            continue outer;
                        }
                    }
                    if (selectors[i].includesFiltersOf(result[j]) &&
                ...
                // This is a distinct selector. Maybe we can find one with the same
                // filters but a different zoomlevel.
                result.push(selectors[i]);
            }
            return result;
        },

        mergeZoomlevels: function(selectors) {
            var result;
            return selectors;
            return result;
        },
        ...
        resolveConditions: function(definitions) {
            ...
            var key = null,
                shortcut = true;
            for (var i = 0; i < definitions.length; i++) {
                if (!(shortcut && (((key == null) && (key = definitions[i].selector.filters[0].key)) ||
                    (key == definitions[i].selector.filters[0].key)) &&
                    (definitions[i].selector.filters.length == 1) &&
                    (definitions[i].selector.filters[0].op.toString() == '='))) {
                    shortcut = false;
                }
            }
            ...
            for (var i = 0; i < definitions.length; i++) {
                ...
                for (var j = 0; j < previousSelectors.length; j++) {
                    for (var k = selectors.length - 1; k >= 0; k--) {
                        ...
                        if (splitSelectors.length >= 1) {
                            selectors[k] = splitSelectors.shift();
                            if (splitSelectors.length) {
                                Array.prototype.push.apply(selectors, splitSelectors);
                            }
                        } else {
                            // This produced an unsound selector. Remove the original
                ...
                selectors = this.mergeZoomlevels(selectors);

                // Now that we split up selectors, eliminated unsound selectors
                // and
