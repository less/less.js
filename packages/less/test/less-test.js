...
export default function(testFilter) {
    ...
    async function testWarnings() {
        ...
        const cases = [
            ...
            ['dash-only variable interpolation warns', '@-: name;\n.@{-} { value: name }', {}, /dash-only variable names @- and @\{-\} are deprecated/, 2],
            ['hyphen-led variable names remain valid without warning', '@-foo: name;\n.a { value: @-foo }', {}, /dash-only variable names @- and @\{-\} are deprecated/, 0],
            ['numeric-leading variable property names warn', '@1: prop;\n.a { @{1}: value }', {}, /Variable names beginning with a number are deprecated/, 2],
            ['dash-only variable property names warn', '@-: prop;\n.a { @{-}: value }', {}, /dash-only variable names @- and @\{-\} are deprecated/, 2],
            ['numeric-leading variable calls warn', '@1: { value: yes; }\n.a { @1(); }', {}, /Variable names beginning with a number are deprecated/, 2],
            ['dash-only variable calls warn', '@-: { value: yes; }\n.a { @-(); }', {}, /dash-only variable names @- and @\{-\} are deprecated/, 2],
            ['dash-only mixin definitions and calls warn', '.-() { value: yes }\n.a { .-() }', {}, /dash-only mixin names \.-\(\) and #-\(\) are deprecated/, 2],
            ['hash dash-only mixin definitions and calls warn', '#-() { value: yes }\n.a { #-() }', {}, /dash-only mixin names \.-\(\) and #-\(\) are deprecated/, 2],
            ['hyphen-led mixin names remain valid without warning', '.-foo() { value: yes }\n.a { .-foo() }', {}, /dash-only mixin names \.-\(\) and #-\(\) are deprecated/, 0],
            ['dynamic @charset interpolation warns', '@encoding: 8;\n@charset "UTF-@{encoding}";', {}, /Dynamic @charset interpolation is deprecated/, 1],
            ['static @charset does not warn', '@charset "UTF-8";', {}, /Dynamic @charset interpolation is deprecated/, 0],
