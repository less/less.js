              ...
              for(let k = 0; k < mapKeyLength; k++) {
                ...
                if (match[2] === 'all' || exactMatch) {
                  ...
                  if (ruleVisible && extendVisible) {
                    ...
                    /** Extend with hidden rulesets copies rules instead of expanding selectors */
                    if (Array.isArray(ruleset.rules)) {
                      ruleset.rules.forEach(rule => {
                        /**
                         * For now, just copy declarations, because copying rulesets is tricky,
                         * as the extendRuleset could be nested inside. 
                         */
                        if (rule instanceof Declaration && !rule.variable) {
                          /** If they're output, this will make them format correctly. */
                          rule.nodeVisible = true;
                          extendRuleset.rules.push(rule);
                        }
                      });
                    }
                  }
