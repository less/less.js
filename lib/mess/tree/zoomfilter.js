...
tree.ZoomFilter = function(op, value, index) {
...
tree.ZoomFilter.prototype.toXML = function(env) {
    ...
    return ranges.map(function(range) {
        return (range[0] > 0 ? '<MaxScaleDenominator>' + tree.ZoomFilter.ranges[range[0]] + '</MaxScaleDenominator>' : '') +
               (range[1] < 22 ? '<MinScaleDenominator>' + tree.ZoomFilter.ranges[range[1] + 1] + '</MinScaleDenominator>' : '');
    });
};
