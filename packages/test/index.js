...
var testMap = [
    ...
    [{math: 'strict', strictUnits: true, sourceMap: true, globalVars: true }, 'sourcemaps/',
        ...
        function(filename, type, baseFolder) {
            ...
            return path.join('sourcemaps', filename) + '.json';
        }],
    ...
    [{paths: [path.join(__dirname, 'data'), path.join(__dirname, 'less', 'import')]}, 'include-path/'],
    [{paths: path.join(__dirname, 'data')}, 'include-path-string/'],
    [{plugin: 'plugins/postprocess/'}, 'postProcessorPlugin/'],
    [{plugin: 'plugins/preprocess/'}, 'preProcessorPlugin/'],
    [{plugin: 'plugins/visitor/'}, 'visitorPlugin/'],
    [{plugin: 'plugins/filemanager/'}, 'filemanagerPlugin/'],
    [{}, 'no-strict-math/'],
    [{}, '3rd-party/']
];
