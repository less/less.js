...
mess.Renderer = function Renderer(env) {
    ...
    return {
        ...
        findSymbolizers: function(definitions) {
            var symbolizers = {};
            for (var i = 0; i < definitions.length; i++) {
                var rules = definitions[i].rules;
                for (var j = 0; j < rules.length; j++) {
                    if (!(rules[j].symbolizer in symbolizers)) {
                        symbolizers[rules[j].symbolizer] = true;
                    }
                }
            }
            return symbolizers;
        ...
        inheritRules: function(definitions) {
            ...
            var next, multiple, newFilters, newRules, updatedFilters, clone;

            for (var i = 0; i < definitions.length; i++) {
                var attachment = definitions[i].attachment;
                ...
                for (var j = i + 1; j < definitions.length; j++) {
                    ...
                    newFilters = definitions[j].filters;
                    newRules = definitions[j].rules;

                    // The current definition might have been split up into
                    // multiple definitions already.
                    for (var k = 0; k < next.length; k++) {
                        updatedFilters = next[k].filters.cloneWith(newFilters)
                        if (updatedFilters) {
                            ...
                            if (previous) {
                                ...
                                clone = next[k].clone(updatedFilters);
                            ...
                            // definition.
                            next[k].addRules(newRules);
                        }
                    }
                }

                for (var j = 0; j < next.length; j++) {
                    existing[attachment][next[j].filters] = next[j];
                    result[attachment].push(next[j]);
                }
            }
        ...
        template: function(err, m, stylesheets, callback) {
            ...
            m.Layer.forEach(function(l) {
                ...
                var symbolizers = that.findSymbolizers(matching);
                var definitions = that.inheritRules(matching);

                for (var symbolizer in symbolizers) {
                    for (var attachment in definitions) {
                        var style = new tree.Style(l.id, symbolizer, attachment, definitions[attachment]);
                        if (style) {
