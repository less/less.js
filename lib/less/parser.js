...
less.Parser = function Parser(env) {
    ...
    function $(tok) {
        ...
        // Non-terminal
        //
        if (typeof tok==='function') return tok.call(parser.parsers);
        //
        // Terminal
        //
        ...
        if (match) {
            ...
            return typeof match=== 'string' ? match : match.length === 1 ? match[0] : match;
        }
    }
    ...
    function basename(pathname) {
        return less.mode === 'node' ? require('path').basename(pathname) : pathname.match(/[^\/]+$/)[0];
    }
        ...
        parse: function (str, callback) {
            ...
            chunks = (function (chunks) {
                ...
                for (var i = 0, c, cc; i < input.length; i++) {
                    skip.lastIndex = i;
                    if ((match = skip.exec(input)) && match.index === i) {
                          i += match[0].length;
                          chunk.push(match[0]);
                    }
                    c = input.charAt(i);
                    ...
                    if ((match = string.exec(input)) && match.index === i) {
                          i += match[0].length;
                          chunk.push(match[0]);
                          c = input.charAt(i);
                    }
                    ...
                    if (!inParam && c === '/') {
                        cc = input.charAt(i + 1);
                        if (cc === '/' || cc === '*') {
                            if ((match = comment.exec(input)) && match.index === i) {
                                  i += match[0].length;
                                  chunk.push(match[0]);
                                  c = input.charAt(i);
                            }
