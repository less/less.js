...
tree.functions = {
    ...
    blend: function (mode, color1, color2, alpha) {
    	...
    	if(Object.keys(modes).join().indexOf(mode.value) < 0) {
    		return new(tree.Color)(color1, alpha || 1.0);
    	}
    	
    	var newRGB = [modes[mode.value](rgb1[0],rgb2[0]),modes[mode.value](rgb1[1],rgb2[1]),modes[mode.value](rgb1[2],rgb2[2])];
