...
mess.Parser = function Parser(env) {
    ...
    return parser = {
            ...
            root.toCSS = (function(evaluate) {
                ...
                return function(options, variables) {
                    ...
                    var returnErrors = 'returnErrors' in options ?
                        options.returnErrors : (env.returnErrors || false);
                    env.errors = returnErrors ? [] : null;

                    try {
                        env.compress = env.compress || false;
                        var css = evaluate.call(this, { frames: [] })
                                          .toCSS([], env);
                    } catch (e) {
                        lines = input.split('\n');
        ...
        parsers: {
            ...
            element: function() {
                var e;
                if (e = $(/^(?:[.#]?)(?:[\w-]|\\(?:[a-fA-F0-9]{1,6} ?|[^a-fA-F0-9]))+/) ||
                        $('*') || $(/^\([^)@]+\)/)) { 
            ...
            //
            // Symbolizers allow adding multiple lines, polygons etc. to an
            // object. There can only be one symbolizer per selector.
            //
            symbolizer: function() {
                var s;
                if (s = $(/^::([\w-])\s*/)) {
                    // There's no object for symbolizer names.
                    return s[1];
                }
            },
            ...
            selector: function() {
                var e, s;
                var symbolizer = null, elements = [];

                while ((e = $(this.element)) || (s = $(this.symbolizer))) {
                    if (e) {
                        elements.push(e);
                    } else if (symbolizer) {
                        throw errorMessage('Encountered second symbolizer', i - 1);
                    } else {
                        symbolizer = s;
                    }
                ...
                if (elements.length > 0 || input.charAt(i) == '[') {
                    return new tree.Selector(elements, symbolizer, memo);
                }
            },
