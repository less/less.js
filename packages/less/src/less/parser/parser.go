...
type Parser struct {
	parserInput  *go_parser.ParserInput
...
type ParseResult struct {
	Error *less.LessError
	Root  *go_parser.Ruleset
}
...
func NewParser(context map[string]any, imports map[string]any, fileInfo map[string]any, currentIndex int) *Parser {
	...
	p := &Parser{
		parserInput:  go_parser.NewParserInput(),
		imports:      imports,
		fileInfo:     fileInfo,
...
func (p *Parser) error(msg string, errorType string) {
	...
	errorDetails := less.ErrorDetails{
	...
	panic(less.NewLessError(errorDetails, contents, filename))
}
...
func (p *Parser) warn(msg string, index any, warnType string) {
	...
	contents, _ := p.imports["contents"].(map[string]string)
	lessError := less.NewLessError(errorDetails, contents, filename)
	logger.Warn(lessError.ToString(nil))
}
...
func (p *Parser) parseInternal(str string, callback func(*less.LessError, *go_parser.Ruleset), data *AdditionalData) {
	...
	p.parserInput.Start(str, chunkInput, func(msg string, index int) {
		panic(less.NewLessError(less.ErrorDetails{
			Index:    index,
			Type:     "Parse",
			Message:  msg,
			Filename: filename,
	...
	if !endInfo.IsFinished {
		...
		err = less.NewLessError(less.ErrorDetails{
			Type:     "Parse",
			Message:  message,
	...
	finish := func(e *less.LessError) {
		if e == nil {
			e = err
		}
		if e == nil {
			if importErr, ok := p.imports["error"].(*less.LessError); ok {
				e = importErr
			}
		}
	...
	if name != nil {
		...
		if !hasDR {
			...
			if value != nil {
				...
				mergeFlag := merge == "+"
				// Create declaration
				decl, err := go_parser.NewDeclaration(name, value, false, mergeFlag, index+p.parser.currentIndex, p.parser.fileInfo, false, isVariable)
				if err != nil {
					return nil
				}
				return decl
...
func (e *EntityParsers) Variable() any {
	...
	if e.parsers.parser.parserInput.CurrentChar() == '@' {
		...
		if nameMatch != nil {
			...
			e.parsers.parser.parserInput.Forget()
			return go_parser.NewVariable(name, index+e.parsers.parser.currentIndex, e.parsers.parser.fileInfo)
		}
	}
	e.parsers.parser.parserInput.Restore("")
	return nil
...
func (p *Parsers) RuleProperty() any {
	...
	if simpleProperty != nil {
		if matches, ok := simpleProperty.([]string); ok && len(matches) > 1 {
			name = append(name, go_parser.NewKeyword(matches[1]))
			p.parser.parserInput.Forget()
			return name
		}
	}
		...
		for k := 0; k < len(name); k++ {
			...
			if str, ok := s.(string); ok {
				if len(str) > 0 && str[0] != '@' && str[0] != '$' {
					name[k] = go_parser.NewKeyword(str)
				} else if len(str) > 0 && str[0] == '@' {
					// Extract variable name from @{var} format
					if len(str) > 3 && str[len(str)-1] == '}' {
						varName := "@" + str[2:len(str)-1]
						name[k] = go_parser.NewVariable(varName, index[k]+p.parser.currentIndex, p.parser.fileInfo)
					} else {
						name[k] = go_parser.NewVariable(str, index[k]+p.parser.currentIndex, p.parser.fileInfo)
					}
				} else if len(str) > 0 && str[0] == '$' {
					// Extract property name from ${prop} format
					if len(str) > 3 && str[len(str)-1] == '}' {
						propName := "$" + str[2:len(str)-1]
						name[k] = go_parser.NewProperty(propName, index[k]+p.parser.currentIndex, p.parser.fileInfo)
					} else {
						name[k] = go_parser.NewProperty(str, index[k]+p.parser.currentIndex, p.parser.fileInfo)
					}
				}
			}
		}
...
func (p *Parsers) AnonymousValue() any {
	...
	if match != nil {
		if matches, ok := match.([]string); ok && len(matches) > 1 {
			return go_parser.NewAnonymous(matches[1], index+p.parser.currentIndex, p.parser.fileInfo, false, false, nil)
		}
	}
	return nil
}
...
func (p *Parsers) DetachedRuleset() any {
	...
	if blockRuleset != nil {
		if ruleset, ok := blockRuleset.(*go_parser.Ruleset); ok {
			return go_parser.NewDetachedRuleset(ruleset.Node, nil)
		}
	}
	return nil
}
...
func (p *Parsers) PermissiveValue(untilTokens *regexp.Regexp, allowComments bool) any {
	...
	for {
		...
		if p.parser.parserInput.Peek(",") {
			value = append(value, go_parser.NewAnonymous(",", p.parser.parserInput.GetIndex()+p.parser.currentIndex, p.parser.fileInfo, false, false, nil))
	...
	if len(value) > 0 {
		expr, err := go_parser.NewExpression(value, false)
		if err != nil {
			return nil
		}
		if done {
		...
		if p.parser.parserInput.PrevChar() == ' ' {
			result = append(result, go_parser.NewAnonymous(" ", index+p.parser.currentIndex, p.parser.fileInfo, false, false, nil))
		}
	}
	...
	if untilValue != nil {
		...
		if valueSlice, ok := untilValue.([]any); ok {
			if len(valueSlice) == 1 && valueSlice[0] == " " {
				p.parser.parserInput.Forget()
				return go_parser.NewAnonymous("", index+p.parser.currentIndex, p.parser.fileInfo, false, false, nil)
			}

			for i, item := range valueSlice {
				if itemSlice, ok := item.([]any); ok {
					// Treat actual quotes as normal quoted values
					result = append(result, go_parser.NewQuoted(itemSlice[0].(string), itemSlice[1].(string), true, index+p.parser.currentIndex, p.parser.fileInfo))
				} else {
					...
					quoted := go_parser.NewQuoted("'", itemStr, true, index+p.parser.currentIndex, p.parser.fileInfo)
					result = append(result, quoted)
				}
			}

			p.parser.parserInput.Forget()
			expr, err := go_parser.NewExpression(result, true)
			if err != nil {
				return nil
			}
			return expr
...
func (p *Parsers) Value() any {
	...
	if len(expressions) > 0 {
		value, err := go_parser.NewValue(expressions)
		if err != nil {
			return nil
		}
		return value
...
func (e *EntityParsers) Quoted(forceEscaped bool) any {
	...
	return go_parser.NewQuoted(string(str[0]), str[1:len(str)-1], isEscaped, index+e.parsers.parser.currentIndex, e.parsers.parser.fileInfo)
}
...
func (e *EntityParsers) Keyword() any {
	...
	if k != nil {
		...
		return go_parser.NewKeyword(kStr)
	}
	return nil
}
...
func (e *EntityParsers) VariableCurly() any {
	...
	if e.parsers.parser.parserInput.CurrentChar() == '@' {
		...
		if curly != nil {
			...
			if len(matches) > 1 {
				return go_parser.NewVariable("@"+matches[1], index+e.parsers.parser.currentIndex, e.parsers.parser.fileInfo)
			}
		}
	}
	return nil
...
func (e *EntityParsers) Property() any {
	...
	if e.parsers.parser.parserInput.CurrentChar() == '$' {
		...
		if name != nil {
			...
			return go_parser.NewProperty(nameStr, index+e.parsers.parser.currentIndex, e.parsers.parser.fileInfo)
		}
	}
	return nil
}
...
func (e *EntityParsers) PropertyCurly() any {
	...
	if e.parsers.parser.parserInput.CurrentChar() == '$' {
		...
		if curly != nil {
			matches := curly.([]string)
			if len(matches) > 1 {
				return go_parser.NewProperty("$"+matches[1], index+e.parsers.parser.currentIndex, e.parsers.parser.fileInfo)
			}
		}
	}
	return nil
...
func (e *EntityParsers) Color() any {
	...
	if e.parsers.parser.parserInput.CurrentChar() == '#' {
		...
		if rgb != nil {
			...
			if len(matches) > 1 {
				e.parsers.parser.parserInput.Forget()
				return go_parser.NewColor(matches[1], 1.0, matches[0])
			}
		}
	}
	e.parsers.parser.parserInput.Restore("")
...
func (e *EntityParsers) Dimension() any {
	...
	if value != nil {
		...
		dimension, _ := go_parser.NewDimension(matches[1], unit)
		return dimension
	}
	return nil
}
...
func (e *EntityParsers) UnicodeDescriptor() any {
	...
	if ud != nil {
		...
		return go_parser.NewUnicodeDescriptor(udStr)
	}
	return nil
}
...
func (e *EntityParsers) JavaScript() any {
	...
	if js != nil {
		...
		e.parsers.parser.parserInput.Forget()
		return go_parser.NewJavaScript(jsStr[:len(jsStr)-1], escape, index+e.parsers.parser.currentIndex, e.parsers.parser.fileInfo)
	}
...
func (p *Parsers) VariableCall() any {
	...
	if p.parser.parserInput.CurrentChar() == '@' {
		...
		if nameMatch != nil {
			...
			if len(lookups) == 0 {
				p.parser.parserInput.Forget()
				return go_parser.NewVariableCall(name, i, p.parser.fileInfo)
			} else {
				p.parser.parserInput.Forget()
				varCall := go_parser.NewVariableCall(name, i, p.parser.fileInfo)
				return go_parser.NewNamespaceValue(varCall, lookups, i, p.parser.fileInfo)
			}
		}
	}
	p.parser.parserInput.Restore("")
...
func (p *Parsers) AtRule() any {
	...
	if rules != nil || (!hasBlock && value != nil && p.parser.parserInput.Char(';') != nil) {
		...
		return go_parser.NewAtRule(name, value, rules, index+p.parser.currentIndex, p.parser.fileInfo, debugInfo, isRooted, nil)
	}
...
func (p *Parsers) Selectors() []any {
	...
	for {
		...
		p.parser.parserInput.CommentsReset()
		if selector, ok := s.(*go_parser.Selector); ok && selector.Condition != nil && len(selectors) > 1 {
			p.parser.error("Guards are only currently allowed on a single selector.", "")
		}
		if p.parser.parserInput.Char(',') == nil {
			break
		}
		if selector, ok := s.(*go_parser.Selector); ok && selector.Condition != nil {
			p.parser.error("Guards are only currently allowed on a single selector.", "")
		}
		// CommentsReset equivalent - clear comment store
		p.parser.parserInput.CommentsReset()
...
func (p *Parsers) Selector(isLess bool) any {
	index := p.parser.parserInput.GetIndex()
	var elements []*go_parser.Element
	var extendList []any
	var e any
	var allExtends []any
	var when bool
...
func (p *Parsers) Extend() []any {
	...
	for {
		...
		for {
			...
			if !first {
				if element, ok := e.(*go_parser.Element); ok && element.Combinator != nil && element.Combinator.Value != "" {
		...
		// Convert []any to []*go_parser.Element
		var elementSlice []*go_parser.Element
		for _, elem := range elements {
			if element, ok := elem.(*go_parser.Element); ok {
...
func (p *Parsers) MediaFeature(syntaxOptions map[string]any) any {
			...
			if p.parser.parserInput.Char(')') != nil {
				if prop != nil && e == nil {
					...
					if condition, ok := prop.(*go_parser.Condition); ok {
						...
						var rangeValue any
						if rangeProp != nil {
							if rangeCond, ok := rangeProp.(*go_parser.Condition); ok {
								rangeOp = rangeCond.Op
								rangeValue = rangeCond.Rvalue
							}
						}
						queryInParens = go_parser.NewQueryInParens(condition.Op, condition.Lvalue, condition.Rvalue, rangeOp, rangeValue, condition.Index)
					}
					nodes = append(nodes, go_parser.NewParen(queryInParens))
					e = prop
				} else if prop != nil && e != nil {
					decl, _ := go_parser.NewDeclaration(prop, e, nil, false, p.parser.parserInput.GetIndex()+p.parser.currentIndex, p.parser.fileInfo, true, false)
					nodes = append(nodes, go_parser.NewParen(decl))
				} else if e != nil {
					nodes = append(nodes, go_parser.NewParen(e))
				} else {
	...
	p.parser.parserInput.Forget()
	if len(nodes) > 0 {
		expr, err := go_parser.NewExpression(nodes, false)
		if err != nil {
			return nil
		}
		return expr
...
func (p *Parsers) Plugin() any {
	...
	if path != nil {
		...
		return go_parser.NewImport(path, nil, options, index+p.parser.currentIndex, p.parser.fileInfo, nil)
	} else {
		p.parser.parserInput.SetIndex(index)
...
func (p *Parsers) Multiplication() any {
	...
	for {
		...
		operation = go_parser.NewOperation(op, operands, isSpaced)
		isSpaced = p.parser.parserInput.IsWhitespace(-1)
		op = ""
	}
...
func (p *Parsers) Condition(needsParens bool) any {
	...
	if logical != "" {
		...
		if next != nil {
			result = go_parser.NewCondition(logical, result, next, p.parser.parserInput.GetIndex()+p.parser.currentIndex, false)
		} else {
			return nil
		}
	}
...
func (p *Parsers) NegatedCondition(needsParens bool) any {
	if p.parser.parserInput.Str("not") != nil {
		...
		if result != nil {
			if condition, ok := result.(*go_parser.Condition); ok {
...
func (p *Parsers) AtomicCondition(needsParens bool, preparsedCond any) any {
	...
	if a != nil {
		...
		if op != "" {
			b = cond()
			if b != nil {
				c = go_parser.NewCondition(op, a, b, index+p.parser.currentIndex, false)
			} else {
				p.parser.error("expected expression", "")
			}
		} else if preparsedCond == nil {
			c = go_parser.NewCondition("=", a, go_parser.NewKeyword("true"), index+p.parser.currentIndex, false)
		}
		return c
	}
	return nil
...
func (m *MixinParsers) Call(inValue bool, getLookup bool) any {
	...
	if elements != nil {
		...
		if inValue || m.parsers.End() {
			...
			mixin, err := go_parser.NewMixinCall(elements, args, index+m.parsers.parser.currentIndex, m.parsers.parser.fileInfo, !getLookup && important)
			if err != nil {
				m.parsers.parser.error(fmt.Sprintf("Failed to create mixin call: %v", err), "Parse")
				return nil
			}
			if len(lookups) > 0 {
				return go_parser.NewNamespaceValue(mixin, lookups, index+m.parsers.parser.currentIndex, m.parsers.parser.fileInfo)
			} else {
				if !hasParens {
...
func (m *MixinParsers) Elements() []*go_parser.Element {
	var elements []*go_parser.Element
	var e string
	var c byte
	var elem *go_parser.Element
	var elemIndex int
...
func (p *Parsers) Expression() any {
	...
	for {
		...
		if e != nil {
			...
			if !p.parser.parserInput.Peek(regexp.MustCompile(`^\/[*/]`)) {
				if p.parser.parserInput.Char('/') != nil {
					entities = append(entities, go_parser.NewAnonymous("/", index+p.parser.currentIndex, p.parser.fileInfo, false, false, nil))
				}
			}
		} else {
			break
	...
	if len(entities) > 0 {
		expr, err := go_parser.NewExpression(entities, false)
		if err == nil {
			return expr
		}
	}
...
func isLineComment(node any) bool {
	if comment, ok := node.(*go_parser.Comment); ok {
...
func (p *Parsers) Import() any {
	...
	if path != nil {
		...
		return go_parser.NewImport(path, featuresValue, options, index+p.parser.currentIndex, p.parser.fileInfo, nil)
	} else {
		p.parser.parserInput.SetIndex(index)
...
func (m *MixinParsers) Definition() any {
	...
	if match != nil {
		...
		if ruleset != nil {
			m.parsers.parser.parserInput.Forget()
			mixinDef, err := go_parser.NewMixinDefinition(name, params, ruleset.([]any), cond, variadic, nil, nil)
			if err != nil {
				return nil
			}
			return mixinDef
...
func (e *EntityParsers) Call() any {
	...
	return go_parser.NewCall(name, args, index+e.parsers.parser.currentIndex, e.parsers.parser.fileInfo)
}
...
func (e *EntityParsers) DeclarationCall() any {
	...
	if rule != nil && value != nil {
		...
		decl, err := go_parser.NewDeclaration(rule, value, nil, false, e.parsers.parser.parserInput.GetIndex()+e.parsers.parser.currentIndex, e.parsers.parser.fileInfo, true, false)
		if err != nil {
	...
	return go_parser.NewCall(validCall, args, index+e.parsers.parser.currentIndex, e.parsers.parser.fileInfo)
}
...
func (e *EntityParsers) URL() any {
	...
	var urlValue any
	if variable, ok := value.(*go_parser.Variable); ok {
		urlValue = variable
	} else if property, ok := value.(*go_parser.Property); ok {
		urlValue = property
	} else {
		urlValue = go_parser.NewAnonymous(value, index, e.parsers.parser.fileInfo, false, false, nil)
	}

	return go_parser.NewURL(urlValue, index+e.parsers.parser.currentIndex, e.parsers.parser.fileInfo, false)
}
...
func (p *Parsers) PrepareAndGetNestableAtRule(atRuleType string, index int, debugInfo map[string]any) any {
	...
	case "Media":
		...
		return go_parser.NewMedia(rules.([]any), featuresValue, index+p.parser.currentIndex, p.parser.fileInfo, debugInfo)
	case "Container":
		var featuresValue any
		if features != nil {
			featuresValue, _ = go_parser.NewValue(features.([]any))
		}
		container, err := go_parser.NewContainer(rules.([]any), featuresValue, index+p.parser.currentIndex, p.parser.fileInfo, debugInfo)
		if err != nil {
			return nil
		}
		return container
...
func (p *Parsers) Combinator() *go_parser.Combinator {
	c := p.parser.parserInput.CurrentChar()
	...
	if c == '/' {
		...
		if slashedCombinator != nil {
			...
			if matches, ok := slashedCombinator.([]string); ok {
				return go_parser.NewCombinator(matches[0])
			}
		}
		p.parser.parserInput.Restore("")
	}
	...
	if c == '>' || c == '+' || c == '~' || c == '|' || c == '^' {
		...
		return go_parser.NewCombinator(combinator)
	} else {
		...
		if p.parser.parserInput.IsWhitespace(0) {
			...
			return go_parser.NewCombinator(" ")
		} else {
			return go_parser.NewCombinator("")
		}
	}
...
func (p *Parsers) Attribute() any {
	...
	return go_parser.NewAttribute(key, opStr, val, cif)
}
...
func (m *MixinParsers) Args(isCall bool) map[string]any {
	...
	for {
		...
		if isCall {
			if expr, ok := arg.(*go_parser.Expression); ok && len(expr.Value) == 1 {
				val = expr.Value[0]
			}
		} else {
			val = arg
		...
		if variable, ok := val.(*go_parser.Variable); ok {
			if m.parsers.parser.parserInput.Char(':') != nil {
				if len(expressions) > 0 {
					if isSemiColonSeparated {
		...
		} else if property, ok := val.(*go_parser.Property); ok {
			if m.parsers.parser.parserInput.Char(':') != nil {
				if len(expressions) > 0 {
					if isSemiColonSeparated {
						m.parsers.parser.error("Cannot mix ; and , as delimiter types", "")
		...
		if hasSep || isSemiColonSeparated {
			...
			if len(expressions) > 1 {
				value, _ = go_parser.NewValue(expressions)
			}
...
func (e *EntityParsers) Arguments(prevArgs []any) []any {
	...
	for {
		if prevArgs != nil {
			...
			if expr, ok := value.(*go_parser.Expression); ok && len(expr.Value) == 1 {
				value = expr.Value[0]
			}
		...
		if e.parsers.parser.parserInput.Char(';') != nil || isSemiColonSeparated {
			...
			if len(argsComma) < 1 {
				...
				argValue, _ = go_parser.NewValue(argsComma)
			}
...
func (e *EntityParsers) Assignment() any {
	...
	if value != nil {
		...
		if matches, ok := key.([]string); ok && len(matches) > 0 {
			return go_parser.NewAssignment(matches[0], value)
		} else if keyStr, ok := key.(string); ok {
			return go_parser.NewAssignment(keyStr, value)
		}
	} else {
		e.parsers.parser.parserInput.Restore("")
	}
...
func (p *Parsers) IeAlpha() []any {
	...
	if value == nil {
		...
		if variable != nil {
			if v, ok := variable.(*go_parser.Variable); ok {
