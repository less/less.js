...
/**
 * A zoom object is sound when there's at least one zoom level that matches.
 */
tree.Zoom.prototype.sound = function() {
    for (var i = 0; i < this.range.length; i++) {
        if (this.range[i]) return true;
    }
};
...
tree.Zoom.prototype.union = function(filter) {
    if (filter.range) {
        for (var i = 0; i < this.range.length; i++) {
            this.range[i] = this.range[i] || filter.range[i];
        }
...
tree.Zoom.prototype.includes = function(other) {
    for (var i = 0; i < this.range.length; i++) {
        if (!this.range[i] && other.range[i]) {
            return false;
        }
    }
    return true;
...
tree.Zoom.prototype.toString = function() {
    return 'Zoom[' + this.range.map(function(v) {
        return v ? 'X' : '.'; }).join('') + ']';
};
