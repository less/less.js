...
tree.Filter.mergable = function(f1, f2) {
    ...
    // When the number of filters differs, they also can't be merged.
    if (filters1.length != filters2.length) return;
    ...
    // There's only one filter difference.
    if (filters1.length == filters2.length == 1) {
